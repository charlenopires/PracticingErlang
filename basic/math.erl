...
init() ->
    ...
    io:format("Sine: sin(pi/2) = ~p~n", [sin(math:pi() / 2)]),
    io:format("Cosine: cos(0) = ~p~n", [cos(0)]),
    io:format("Tangent: tan(pi/4) = ~p~n", [tan(math:pi() / 4)]),
    io:format("Factorial: fac(5) = ~p~n", [fac(5)]),
    io:format("Sum of the numbers in the list: ~p~n", SumList),
    io:format("Média: mean([1, 2, 3, 4, 5]) = ~p~n", [mean([1, 2, 3, 4, 5])]),
    io:format("Mediana: median([1, 2, 3, 4, 5]) = ~p~n", [median([1, 2, 3, 4, 5])]),
    ok.